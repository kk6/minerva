...
以下のコンポーネントは、カバレッジ測定の対象外または低カバレッジが許容されています：

- サーバー起動コード（server.py, __main__.py）: これらはインフラ層のコードで、ビジネスロジックを含まない
- 実行時のみ使用されるユーティリティコード

カバレッジレポートは定期的に更新され、`htmlcov/` ディレクトリに保存されます。
