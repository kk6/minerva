...
#### 2.1.4.1 セマンティック検索機能（v0.15.0実装完了）
- **セマンティック検索**: 384次元ベクター埋め込みによる意味的類似性検索
- **関連ノート発見**: 指定したクエリと意味的に関連するノートの発見
- **高精度類似度計算**: コサイン類似度による精密な類似度判定
- **柔軟な検索オプション**: 類似度閾値、結果件数制限、ディレクトリ指定検索

**実装状況（v0.15.0で完了）**:
- ✅ **インフラ構築**: `vector/`モジュール構造と抽象クラス設計
- ✅ **埋め込み生成**: SentenceTransformerProviderでall-MiniLM-L6-v2モデル実装（384次元）
- ✅ **ベクターインデックス**: DuckDB VSS拡張でHNSWインデックス実装
- ✅ **セマンティック検索**: SearchOperationsサービスへの完全統合
- ✅ **MCP統合**: semantic_search, build_vector_index, get_vector_index_statusツール実装
- ✅ **デバッグ支援**: debug_vector_schema, reset_vector_database, build_vector_index_batchツール
- ✅ **設定管理**: オプション機能として環境変数で制御（VECTOR_SEARCH_ENABLED=true）
- ✅ **テストインフラ**: 包括的な単体テストと統合テスト
- ✅ **オプショナル依存関係管理**: 適切なエラーメッセージとlazy loading

**技術スタック**: DuckDB 1.1.3+ (VSS拡張), sentence-transformers 3.3.1+, numpy 1.24.0+ (オプション依存)
...
4. Claude Desktopへのインストール: `uv run mcp install src/minerva/server.py:mcp -f .env`
