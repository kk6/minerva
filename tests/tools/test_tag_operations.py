...
from minerva.tools import (
    ...
    list_all_tags, ListAllTagsRequest,
    find_notes_with_tag, FindNotesWithTagRequest,
    rename_tag, RenameTagRequest,
    _normalize_tag, _validate_tag # For direct testing if needed, or for reference
)
from minerva.config import DEFAULT_NOTE_DIR # For creating notes in default dir within mock_vault
...
def mock_vault_path(tmp_path_factory):
    ...
    (notes_dir / "note_malformed_frontmatter.md").write_text(NOTE_MALFORMED_FRONTMATTER_CONTENT)
    ...
    with mock.patch("minerva.config.VAULT_PATH", vault_dir):
        yield vault_dir
...
def test_validate_tag():
    assert _validate_tag("validtag") == True
    assert _validate_tag("valid-tag") == True
    assert _validate_tag("valid_tag") == True
    assert _validate_tag("tag with space") == True # Spaces are allowed by _validate_tag, normalization handles them for storage
    assert _validate_tag("") == False # Empty string is invalid
    assert _validate_tag("tag,with,comma") == False
    assert _validate_tag("tag<invalid>") == False
    assert _validate_tag("tag/slash") == False
    assert _validate_tag("tag?question") == False
    assert _validate_tag("tag'quote") == False
    assert _validate_tag("tag\"doublequote") == False
...
def test_get_tags_file_not_exist(mock_vault_path: Path):
    request = GetTagsRequest(filepath=str(mock_vault_path / DEFAULT_NOTE_DIR / "non_existent_note.md"))
    tags = get_tags(request)
    assert tags == []
...
def test_get_tags_by_filename(mock_vault_path: Path):
    # Assuming note1.md is in DEFAULT_NOTE_DIR relative to VAULT_PATH
    request = GetTagsRequest(filename="note1.md", default_path=DEFAULT_NOTE_DIR)
    tags = get_tags(request)
    assert sorted(tags) == sorted(["TagA", "tagB"])

def test_get_tags_by_filename_in_subdir(mock_vault_path: Path):
    request = GetTagsRequest(filename="subdir/note_in_subdir.md", default_path=DEFAULT_NOTE_DIR)
    tags = get_tags(request)
    assert tags == ["SubTag"]
...
def test_add_tag_to_note_with_existing_tags(mock_vault_path: Path):
    ...
    assert sorted(post.metadata["tags"]) == sorted(["taga", "tagb", "tagc"]) # add_tag normalizes existing tags on rewrite
    assert post.metadata.get("created") == original_created
    assert "updated" in post.metadata

def test_add_tag_already_exists(mock_vault_path: Path):
    note_path = mock_vault_path / DEFAULT_NOTE_DIR / "note1.md" # Has TagA, tagB
    request = AddTagRequest(filepath=str(note_path), tag="taga") # Adding "taga" which normalizes to "taga"
    modified_path = add_tag(request)
    post = read_md_file(modified_path)
    
    # _generate_note_metadata normalizes all tags and de-duplicates
    assert sorted(post.metadata["tags"]) == sorted(["taga", "tagb"])
    assert "updated" in post.metadata # File is still re-written and 'updated' timestamp changed
...
def test_add_tag_file_not_found_error(mock_vault_path: Path):
    with pytest.raises(FileNotFoundError):
        request = AddTagRequest(filepath=str(mock_vault_path / "non_existent.md"), tag="ValidTag")
        add_tag(request)
...
def test_remove_existing_tag(mock_vault_path: Path):
    ...
    request = RemoveTagRequest(filepath=str(note_path), tag="TagA")
    modified_path = remove_tag(request)
    post = read_md_file(modified_path)

    assert "tags" in post.metadata
    assert post.metadata["tags"] == ["tagb"] # _generate_note_metadata normalizes
    assert post.metadata.get("created") == original_created
    assert "updated" in post.metadata
    assert post.metadata.get("custom_field") == "preserve_me" # Check unrelated metadata
...
def test_remove_non_existent_tag(mock_vault_path: Path):
    ...
    assert sorted(post.metadata["tags"]) == sorted([_normalize_tag(t) for t in original_tags])
...
def test_remove_tag_from_note_with_empty_tags_list(mock_vault_path: Path):
    note_path = mock_vault_path / DEFAULT_NOTE_DIR / "note_empty_tags.md" # Has tags: []
...
def test_remove_tag_file_not_found(mock_vault_path: Path):
    with pytest.raises(FileNotFoundError):
        request = RemoveTagRequest(filepath=str(mock_vault_path / "non_existent.md"), tag="AnyTag")
        remove_tag(request)

def test_remove_tag_needs_normalization_for_match(mock_vault_path: Path):
    note_path = mock_vault_path / DEFAULT_NOTE_DIR / "note2.md" # Has "  Whitespace Tag  "
    request = RemoveTagRequest(filepath=str(note_path), tag="whitespace tag") # Normalized form
    modified_path = remove_tag(request)
    post = read_md_file(modified_path)
...
def test_list_all_tags_basic_scenario(mock_vault_path: Path):
    ...
    request = ListAllTagsRequest(directory=str(mock_vault_path)) # Scan whole mock vault
...
def test_list_all_tags_non_existent_directory(mock_vault_path: Path):
    with pytest.raises(FileNotFoundError):
        request = ListAllTagsRequest(directory=str(mock_vault_path / "non_existent_dir"))
...
"""
    ...
    expected = sorted(["taga", "tagb", "tagc", "whitespace tag", "subtag", "actual_tag"])
...
def test_find_notes_with_tag_non_existent_tag(mock_vault_path: Path):
    request = FindNotesWithTagRequest(tag="NonExistentTag", directory=str(mock_vault_path))
    found_files = find_notes_with_tag(request)
    assert found_files == []
...
def test_find_notes_with_tag_needs_normalization(mock_vault_path: Path):
    ...
    request = FindNotesWithTagRequest(tag="  WhItEsPaCe tAg  ", directory=str(mock_vault_path))
    found_files = find_notes_with_tag(request)
...
def test_find_notes_with_tag_non_existent_directory(mock_vault_path: Path):
    with pytest.raises(FileNotFoundError):
        request = FindNotesWithTagRequest(tag="anytag", directory=str(mock_vault_path / "non_existent_dir"))
        find_notes_with_tag(request)

def test_find_notes_with_empty_normalized_tag(mock_vault_path: Path):
    request = FindNotesWithTagRequest(tag="   ", directory=str(mock_vault_path)) # Tag normalizes to empty
    found_files = find_notes_with_tag(request)
    assert found_files == []
...
def test_find_notes_with_tag_scoped_to_subdir(mock_vault_path: Path):
    ...
    found_files = find_notes_with_tag(request)
    assert found_files == [str(mock_vault_path / DEFAULT_NOTE_DIR / "subdir" / "note_in_subdir.md")]
...
def test_rename_tag_basic(mock_vault_path: Path):
    ...
    note2_path = mock_vault_path / DEFAULT_NOTE_DIR / "note2.md" # Has tagB, TagC, Whitespace Tag
    
    original_note1_post = read_md_file(note1_path)
    original_note1_created = original_note1_post.metadata.get("created")
    original_note1_updated = original_note1_post.metadata.get("updated") # Might not exist initially

    request = RenameTagRequest(old_tag="TagA", new_tag="NewTagForA")
    ...
    assert post1.metadata.get("updated") != original_note1_updated # Should have changed

    # Check note2.md (should be unchanged)
    post2 = read_md_file(note2_path)
    assert sorted(_normalize_tag(t) for t in post2.metadata["tags"]) == sorted(["tagb", "tagc", "whitespace tag"])


def test_rename_tag_case_insensitive_old_tag(mock_vault_path: Path):
    note1_path = mock_vault_path / DEFAULT_NOTE_DIR / "note1.md" # Has TagA, tagB
    request = RenameTagRequest(old_tag="taga", new_tag="StillNewTagA") # old_tag is lowercase
    modified_files = rename_tag(request)
...
def test_rename_tag_new_tag_casing_preserved_on_write_normalized_on_logic(mock_vault_path: Path):
    ...
    request = RenameTagRequest(old_tag="TagA", new_tag="BrandNewTagWithCase")
    rename_tag(request)
    post1 = read_md_file(note1_path)
...
def test_rename_tag_to_existing_tag_merges(mock_vault_path: Path):
    note1_path = mock_vault_path / DEFAULT_NOTE_DIR / "note1.md" # Has TagA, tagB
    request = RenameTagRequest(old_tag="TagA", new_tag="tagB") # Rename TagA to tagB
    modified_files = rename_tag(request)

    assert str(note1_path) in [str(p) for p in modified_files]
    post1 = read_md_file(note1_path)
    # _generate_note_metadata de-duplicates normalized tags
    assert sorted(post1.metadata["tags"]) == ["tagb"] # Only "tagb" should remain (normalized)
...
def test_rename_tag_non_existent_old_tag(mock_vault_path: Path):
    ...
    assert note1_path.read_text() == original_content # File should not have been re-written
...
def test_rename_tag_non_existent_directory(mock_vault_path: Path):
    with pytest.raises(FileNotFoundError):
        request = RenameTagRequest(old_tag="TagA", new_tag="NewTag", directory=str(mock_vault_path / "non_existent_dir"))
        rename_tag(request)
...
def test_rename_tag_across_multiple_files(mock_vault_path: Path):
    ...
    request = RenameTagRequest(old_tag="tagB", new_tag="UniversalTagB")
    modified_files = rename_tag(request)
    ...
    # Check note1
    post1 = read_md_file(note1_path)
    assert sorted(post1.metadata["tags"]) == sorted([_normalize_tag("TagA"), "universaltagb"])
    # Check note2
    post2 = read_md_file(note2_path)
    assert sorted(post2.metadata["tags"]) == sorted([_normalize_tag("TagC"), _normalize_tag("Whitespace Tag"), "universaltagb"])
    # Check root_note
    post_root = read_md_file(root_note_path)
    assert sorted(post_root.metadata["tags"]) == sorted([_normalize_tag("RootTag"), "universaltagb"])
...
def test_rename_tag_scoped_to_directory(mock_vault_path: Path):
    ...
    request = RenameTagRequest(old_tag="tagB", new_tag="NewTagBInNotesDir", directory=notes_dir_path_str)
    modified_files = rename_tag(request)
