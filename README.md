...
- 類似ノートの検索（find_similar_notes）
- 重複ノートの検出（find_duplicate_notes）- セマンティック類似度による重複コンテンツ発見
...
- [x] **セマンティック検索機能** - ベクター埋め込みによる意味的類似度検索でノート発見支援（v0.15.0で実装完了：埋め込み生成・ベクターインデックス・DuckDB VSS統合・セマンティック検索・MCP統合）
...
- [x] **ノート間の重複検出** - 類似内容のノートを検出して統合を提案
- [ ] **定期レビュー支援** - 最終更新から一定期間経過したノートをリストアップ
- [x] **ノートのマージ機能** - 複数のノートを1つに統合
